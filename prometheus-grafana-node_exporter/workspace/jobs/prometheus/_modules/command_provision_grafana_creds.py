...
def is_password_available():
    ...
    r = requests.get("http://localhost:8080/kv",
                     json={"namespace": "vars/job/prometheus", "key": "grafana_password"},
                     headers = {"X-ALLOCATION-ID":allocation_id}, timeout=5)
    return r.status_code == 200

def main():
    kv_put("grafana_user", "grafana")
    if not is_password_available():
        password = str(generate_password())
        hashed_password = bcrypt.hashpw(password.encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
        kv_put("grafana_password", password)
        kv_put("grafana_password_hash", hashed_password)
